...
Run locally:  python3 backend/train_xgboost.py  (from project root)
Run on Modal: modal run backend/train_xgboost.py
"""
import os
import numpy as np
import pandas as pd
...
# 15 leakage-free features (price_change_1m and price_change_3m excluded)
FEATURES = [
    ...
    "mortgage_rate_30y",   # 30-year fixed rate (FRED MORTGAGE30US)
]
...
def train():
    ...
    df = pd.read_csv(os.path.join(DATA_DIR, "dallas_clean.csv"))
    df["Date"] = pd.to_datetime(df["Date"])
    df = df.sort_values(["ZipCode", "Date"]).reset_index(drop=True)

    # Engineer 3-month and 6-month forward targets
    ...
    for label, target_col in horizons:
        ...
        r  = _rmse(y_test, preds)
        m  = mean_absolute_error(y_test, preds)
        mp = _mape(y_test, preds)
        ...
        out_path = os.path.join(MODEL_DIR, f"xgboost_{label}.pkl")
        artifact = {
            "model": model,
            "features": FEATURES,
            "horizon": label,
    ...
    # Latest data snapshot for the predict endpoint
    latest = df.sort_values("Date").groupby("ZipCode").last().reset_index()
    latest_path = os.path.join(MODEL_DIR, "latest_data.pkl")
...
# ---------------------------------------------------------------------------
# Modal entrypoint — run with: modal run backend/train_xgboost.py
# ---------------------------------------------------------------------------
try:
    ...
    _modal_app = _modal.App("train-xgboost", image=_image)
