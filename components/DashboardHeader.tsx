...
export default function DashboardHeader({ zipcode, city, bedrooms, bathrooms, currentPrice, yoyChange }: Props) {
  const isUp = yoyChange >= 0
  const yoyAbs = Math.abs(yoyChange).toFixed(1)
  const bedroomLabel = bedrooms === 5 ? '5+ bed' : `${bedrooms}-bed`
  const bathroomLabel = bathrooms === 4 ? '4+ bath' : `${bathrooms}-bath`
    ...
    <div className="flex items-end justify-between flex-wrap gap-4 pb-6 border-b border-gray-200/80">
      <div>
        ...
        <h1 className="font-serif text-3xl text-gray-950">
          {bedroomLabel.charAt(0).toUpperCase() + bedroomLabel.slice(1)}, {bathroomLabel} Homes
        </h1>
      </div>
